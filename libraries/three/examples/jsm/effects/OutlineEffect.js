...
class OutlineEffect {
	...
	constructor( renderer, parameters = {} ) {
		 ...
		 */
		this.render = function ( scene, outlineList, camera ) {
			...
			this.renderOutline( scene, outlineList, camera );
		 ...
		 * @param {Camera} camera - The camera.
		 */
		this.renderOutline = function ( scene, outlineList, camera ) {
			...
			for(let i = 0, max = outlineList.length; i < max; i++){
				let mesh = outlineList[i];
				mesh.traverse(setOutlineMaterial);
			}
			...
			for(let i = 0, max = outlineList.length; i < max; i++){
				let mesh = outlineList[i];
				mesh.traverse(restoreOriginalMaterial);
			}
